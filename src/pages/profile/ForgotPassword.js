...
export default function ForgotPassword() {
    ...
    const submit = () => {
        ...
        console.log(apiData)
        API.login('identity/password_reset/confirm/', apiData).then((response) => {
                    ...
                    <Form onSubmit={handleSubmit(submit)}>
                        <Row className="mb-3">
                            <Form.Group as={Col} controlId="">
                                ...
                                <Form.Control type="password" placeholder="New Password" name="password" defaultValue={data?.password} {...passwordField} onChange={(e) => { passwordField.onChange(e); handleChange(e); }} />
                                {errors.password && (<span className="validationError">New Password is required!</span>)}
                            </Form.Group>
                        </Row>
                        <Row className="mb-3">
                            <Form.Group as={Col} controlId="">
                                <Form.Label className='required-field'>Confirm New Password</Form.Label>
                                <Form.Control type="password" placeholder="Confirm New Password" name="confirm_password" defaultValue={data?.confirm_password} {...confirmPasswordField} onChange={(e) => { confirmPasswordField.onChange(e); handleChange(e); }} />
